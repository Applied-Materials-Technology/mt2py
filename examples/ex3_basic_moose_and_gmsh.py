...
sources = ['gmsh','moose']
output_names = ['-exportpath','Outputs/file_base']

oc = OutputConfig(parent,sources,output_names)

# Set up command line
gmsh_cl = CommandLineConfig('gmsh','python',Path('scripts/ex2_gmsh.py'))
moose_cl = CommandLineConfig('moose','sloth-opt',Path('scripts/ex3_linear_elastic_mesh.i'),'-i')
